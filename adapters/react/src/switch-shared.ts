// Switch — the React lifecycle half. The logic (the lastNativeReport reducer, valueFromChange,
...
export function createSwitch(platform: ISwitchHostPlatform): FC<ISwitchProps> {
  return (rawProps) => {
    ...
    const { value, onValueChange, onChange, disabled, trackColor, thumbColor, ios_backgroundColor, style, ...passthrough } =
      props
