...
export const RefreshControl = defineComponent({
  ...
  setup(_props, { attrs: rawAttrs, slots }: SetupContext) {
    return () => {
      ...
      if (nativeProps.enabled !== undefined) dlog(`RefreshControl enabled=${String(nativeProps.enabled)} (Android-only)`)
      ...
      return h('symbiote-refresh-control', nativeProps, slots.default !== undefined ? slots.default() : undefined)
    }
  },
})
