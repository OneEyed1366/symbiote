...
// a scroll view wraps a content view that holds the children (RN's ScrollView.js shape). All
  ...
  return defineComponent({
    ...
    setup(_props, { slots, attrs: rawAttrs, expose }: SetupContext) {
      ...
      return () => {
        ...
        const stickyHeaderIndices = isNumberArray(attrs.stickyHeaderIndices) ? attrs.stickyHeaderIndices : undefined
        ...
        const outerProps: Record<string, unknown> = { ...resolveAccessibilityProps(forwardAttrs(attrs)) }
        ...
        if (hasStickyHeaders) {
          ...
          if (nativeStickyAvailable) {
            ...
            outerProps.onScroll = animatedEvent(
              ...
              userOnScroll === undefined ? undefined : { listener: (...args) => forwardScrollEvent(userOnScroll, args) },
            )
        ...
        // false additionally preserves the cell views maintainVisibleContentPosition / snapToAlignment
        ...
        if (attrs.maintainVisibleContentPosition !== undefined || attrs.snapToAlignment !== undefined) {
