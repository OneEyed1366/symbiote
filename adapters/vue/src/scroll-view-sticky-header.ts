...
//
// VERDICT (source-based): RN does stickiness PURELY IN JS. ScrollView.js (render, ~line 1690)
...
// 1095). The native Fabric scroll view does NOT honor the index array on its own — forwarding
...
// tracks scroll on the UI thread (no JS jitter). Ported faithfully from ScrollViewStickyHeader.js,
...
export const ScrollViewStickyHeader = defineComponent({
  ...
  setup(_props, { attrs, slots }: SetupContext) {
    ...
    // fallback degrades gracefully if the invariant (wrapStickyHeaders always supplies it) ever breaks.
    ...
    // own onLayout — RN ScrollViewStickyHeader.js._onLayout order.
    ...
    return () => {
      ...
      const passthroughAnimatedPropExplicitValues =
        translateY.value !== null ? { style: { transform: [{ translateY: translateY.value }] } } : null
      ...
      return h(
        Animated.View,
        {
          style: { transform: [{ translateY: animatedTranslateY.value }], zIndex: STICKY_HEADER_Z_INDEX },
          onLayout,
          collapsable: false,
