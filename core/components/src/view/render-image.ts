import { dlog, flattenStyle, getNativeModule, type IStyleProp, type ISymbioteEvent, type IViewStyle } from '@symbiote/engine'
...
function expandSrcSet(srcSet: string, view: IImageViewProps, headers: Record<string, string>): IImageSource[] {
  ...
  if (useSrcForDefaultScale && view.src !== undefined) {
    sources.push({ uri: view.src, scale: 1, width: view.width, height: view.height, ...{ headers } })
  }
...
function readStyleString(style: IStyleProp<IViewStyle> | undefined, key: 'resizeMode' | 'tintColor'): string | undefined {
